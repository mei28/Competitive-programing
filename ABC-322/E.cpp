...
pair<int, vector<int>> search_first_half(int mask, int N, int K, vector<vector<int>>& plans) {
    int cost = 0;
...
pair<int, vector<int>> search_second_half(int mask, int N, int K, vector<vector<int>>& plans) {
    int cost = 0;
    vector<int> params(K, 0);
    for (int i = N / 2; i < N; ++i) {
        if (mask & (1 << (i - N / 2))) {
...
int main() {
    ...
    vector<pair<vector<int>, int>> sorted_first_half(first_half.begin(), first_half.end());
    sort(sorted_first_half.begin(), sorted_first_half.end(), [](const auto& a, const auto& b) {
