class BalancingTree:
            ...
            if v == nd.value:
                ...
                if mi < nd.pivot:
                    ...
                    if nd.left:
                        ...
                        p = nd.pivot
                        nd.left = self.node(
                            mi, p - (p & -p) // 2, self.identifying_bit_length
                        )
                        break
                else:
                    nd.value = mi
                    if nd.right:
                        ...
                        p = nd.pivot
                        nd.right = self.node(
                            ma, p + (p & -p) // 2, self.identifying_bit_length
                        )
                        break
    ...
    def sum_less_than(self, v):
        ...
        while 1:
            if nd.value < v:
                re += (nd.left.sum if nd.left else 0) + (
                    nd.value - 1 >> self.identifying_bit_length
                )
                if nd.right:
                    nd = nd.right
    ...
    def __getitem__(self, k):
        ...
        if l >= r:
            return 0
        return self.sum_of_first_k_elements(r) - (
            self.sum_of_first_k_elements(l) if l else 0
        )
